...
### JSON repair
@app.post("/json-repair")
async def repair_json(json_string: str = Body(..., embed=True, description="Possibly malformed JSON string to repair")):
    """Repair a malformed JSON string using the json_repair module and return the repaired JSON as a string.
    ...
    Returns the repaired JSON as a properly formatted JSON string.
    """
    try:
        # Import lazily so module is only required for this endpoint
        import json_repair
    except ImportError:
        raise HTTPException(
            status_code=503,
            detail={
                "error": "json_repair library is not installed",
                "message": "Please install 'json-repair' to use this endpoint",
                "install_command": "pip install json-repair"
            }
        )

    try:
        # Use json_repair to parse and repair the malformed JSON
        repaired_data = json_repair.loads(json_string)
        ...
        raise HTTPException(
            ...
            detail={
                ...
                "message": str(e),
                "input_preview": json_string[:100] + "..." if len(json_string) > 100 else json_string
            }
        )
...
when working on embeddings - you should nto worry about storing embeddings - you can just use the similarity endpoint like: curl -X 'POST' 

'https://refresh.penguinai.co/backend_apis/embeddings/similarity' 
...
"text2": "he is kishore",
"model_name": "all-MiniLM-L6-v2"
}' to get the similarity score to identify the text that might contain the answer

the document that has the highest similarity should be used to answer the question by giving the doc as context to llm - DO not keep any threshold for similarity

When performing question answering, you should use embeddings only when the the text has more than 10000 WORDS - at that time you would split tetx for every 10K words, chunk and identify the most relevant chunk to answer question and then answer question

ALWAYS give Clear example of expected JSON structure within the prompt when fetching a JSON

When you receive a JSON response from LLM ALWAYS call the json-repair endpoint
