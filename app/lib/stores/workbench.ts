...
import { addTestReport, setTestRunning } from './tests';
...
export class WorkbenchStore {
  ...
  #compilationErrorHandler: ((errorData: any) => void) | undefined;
  constructor() {
    if (import.meta.hot) {
  ...
  setCompilationErrorHandler(handler: ((errorData: any) => void) | undefined) {
    this.#compilationErrorHandler = handler;
  }
  ...
  hasBackendDirectory(): boolean {
    ...
    // Check if backend/main.py exists (minimum requirement for backend mode)
    return !!files['backend/main.py'] || !!files['/backend/main.py'];
  }
  ...
  runTests(testType: 'browser' | 'api' | 'accessibility' | 'performance' = 'browser') {
    // Get the current artifact
    const artifacts = this.artifacts.get();
    const currentArtifact = Object.values(artifacts).find(a => !a.closed);
    
    if (!currentArtifact) {
      console.warn('No active artifact to test');
      return;
    }
    ...
    const testAction = {
      ...
      config: {
        checkSEO: testType === 'browser',
        checkImages: testType === 'browser',
        wcagLevel: testType === 'accessibility' ? 'AA' : undefined,
      },
    };

    // Set test running state
    setTestRunning(true, testAction);

    // Add action to runner
    const actionId = `test-${Date.now()}`;
    currentArtifact.runner.addAction({
      actionId,
    ...
    // Run the test
    currentArtifact.runner.runAction({
      actionId,
      messageId: currentArtifact.messageId,
      action: testAction,
    });
  ...
  addArtifact({ messageId, title, id, type }: ArtifactCallbackData) {
    ...
    const runner = new ActionRunner(
        ...
        (report: TestReport) => {
          ...
          addTestReport(`${id}-${Date.now()}`, report);
          setTestRunning(false);
          
          // Show alert if tests failed
    ...
    // Set compilation error handler
    runner.onCompilationError = (errorData) => {
      if (this.#reloadedMessages.has(messageId)) {
        return;
      }
      // Forward to the registered handler
      if (this.#compilationErrorHandler) {
        this.#compilationErrorHandler(errorData);
      }
    };
