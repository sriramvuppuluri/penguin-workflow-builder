import type { ActionType, BoltAction, BoltActionData, FileAction, ShellAction, SupabaseAction, TestAction, ValidateAction, FeedbackAction } from '~/types/actions';
...
export interface TestCallbackData {
  messageId: string;
  report: TestReport;
  feedback: string;
}

export type TestCallback = (data: TestCallbackData) => void;
...
export interface ParserCallbacks {
  ...
  onTestResult?: TestCallback;
  onTestFeedback?: TestCallback;
}
...
export class StreamingMessageParser {
  ...
  /**
   * Detect test commands in user messages
   */
  static detectTestCommand(message: string): TestAction | null {
    const lowerMessage = message.toLowerCase();
    
    // Test command patterns
    const patterns = [
      { regex: /(?:run|execute|perform|do) (?:all )?tests?/i, type: 'browser' as const },
      { regex: /test (?:the |this )?(?:app|application|site|website)/i, type: 'browser' as const },
      { regex: /check accessibility/i, type: 'accessibility' as const },
      { regex: /test accessibility/i, type: 'accessibility' as const },
      { regex: /check performance/i, type: 'performance' as const },
      { regex: /test performance/i, type: 'performance' as const },
      { regex: /test (?:the )?api/i, type: 'api' as const },
      { regex: /validate (?:the )?(?:app|application)/i, type: 'browser' as const },
      { regex: /verify (?:the )?(?:app|application) works/i, type: 'browser' as const },
    ];
    ...
    for (const { regex, type } of patterns) {
      if (regex.test(message)) {
        return {
          ...
          config: {
            checkSEO: type === 'browser',
            checkImages: type === 'browser',
            wcagLevel: type === 'accessibility' ? 'AA' : undefined,
          },
        };
      }
    }
