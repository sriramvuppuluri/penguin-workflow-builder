...
import { getBackendPrompt } from '~/lib/common/prompts/backend-prompt';
...
export async function streamText(props: {
  ...
  if (promptLogger.isEnabled()) {
    promptLogger.logSystemPrompt(systemPrompt, {
    ...
    promptLogger.logMessages(processedMessages);
  }
  ...
  const streamParams = {
    ...
    system: chatMode === 'backend' ? (() => {
      // Extract database type from the last user message if it mentions database
      const lastUserMessage = processedMessages.filter(m => m.role === 'user').slice(-1)[0];
      let databaseType = 'SQLite'; // default
      if (lastUserMessage?.content) {
        const content = lastUserMessage.content.toLowerCase();
        if (content.includes('sqlite')) databaseType = 'SQLite';
        else if (content.includes('postgresql')) databaseType = 'PostgreSQL';
        else if (content.includes('json files')) databaseType = 'JSON';
        else if (content.includes('csv files')) databaseType = 'CSV';
      }
      return getBackendPrompt(undefined, databaseType);
  ...
  // Log streaming response if enabled
  if (promptLogger.isEnabled()) {
    let fullResponse = '';
    
    // Create a wrapped stream to capture the response
    const originalAsyncIterable = result.textStream[Symbol.asyncIterator];
    result.textStream[Symbol.asyncIterator] = async function* () {
      const iterator = originalAsyncIterable.call(result.textStream);
      for await (const chunk of iterator) {
        fullResponse += chunk;
        promptLogger.logStreamChunk(chunk);
        yield chunk;
      }
      
      // Log complete response when streaming is done
      if (fullResponse) {
        promptLogger.logAssistantResponse(fullResponse, {
          model: modelDetails.name,
          provider: provider.name,
          chatMode
        });
