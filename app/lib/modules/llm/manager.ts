...
export class LLMManager {
  ...
  getDefaultProvider(): BaseProvider {
    // Always return AmazonBedrock provider for Claude Sonnet 4
    const bedrockProvider = this._providers.get('AmazonBedrock');
    
    if (bedrockProvider) {
      return bedrockProvider;
    }

    // Fallback to first provider if AmazonBedrock is not available
