...
import { ErrorMonitor } from '~/lib/runtime/error-monitor';
...
export class BoltShell {
  ...
  async waitTillOscCode(waitCode: string) {
    ...
    while (true) {
      ...
      const errors = errorMonitor.monitorOutput(text);
      if (errors.length > 0) {
        console.log('[Shell] Compilation errors detected:', errors);
        // Store errors for later processing
        this.lastDetectedErrors = errors;
      }
